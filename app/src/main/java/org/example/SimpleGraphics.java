...
public class SimpleGraphics extends Application {
    ...
    /**
     * AI Attribution: Generated with Claude.
     * Prompt: "Use lines to add internal texture to the flowing cape, and some inner fill
     * for shadows — the edge is good, but the plain solid fill can be improved. Derive both
     * the shadow color and the crease line color automatically from the fill color."
     */
    public static void flowingCape(double x, double y, double width, double length, double waveAmount, int numWaves, String color) {
        double waveStrength = waveAmount * (width * 0.14); // how far the tails jut out and notch in

        // The cape attaches along the right side (at the shoulders) and sweeps left
        double neckX = x, neckY = y;
        double hemX = x, hemY = y + length * 0.55;

        // Leading tip: the far top-left corner the wind pulls the fabric toward
        double leadTipX = x - width;
        double leadTipY = y + length * (0.20 - waveAmount * 0.10);

        // Trailing end: bottom of the ragged left edge, swept back toward the body
        double tailEndX = x - width * (0.75 - waveAmount * 0.20);
        double tailEndY = y + length * 0.95;

        // Shading colors derived from the fill so the cape reads as folded fabric
        Color base = toColor(color);
        Color shadow = base.darker().deriveColor(0, 1, 1, 0.55);
        Color crease = base.brighter().deriveColor(0, 1, 1, 0.45);

        gc.setFill(base);
        gc.beginPath();
        gc.moveTo(neckX, neckY);

        // Top edge: one long arc billowing upward as it sweeps out to the leading tip
        gc.bezierCurveTo(x - width * 0.30, y - length * 0.20,
                        x - width * 0.70, y - length * 0.05,
                        leadTipX, leadTipY);

        // Trailing edge: alternating notches and points, one per tail
        double prevX = leadTipX, prevY = leadTipY;
        for (int i = 1; i <= numWaves; i++) {
            double t = (double) i / numWaves;
            double tipX = leadTipX + (tailEndX - leadTipX) * t - waveStrength;
            double tipY = leadTipY + (tailEndY - leadTipY) * t;
            double notchX = (prevX + tipX) / 2 + waveStrength * 1.6; // curls back inward between tails
            double notchY = (prevY + tipY) / 2;
            gc.quadraticCurveTo(notchX, notchY, tipX, tipY);
            prevX = tipX;
            prevY = tipY;
        }

        // Bottom edge: long arc sagging away and sweeping back to the hem at the body
        gc.bezierCurveTo(x - width * 0.55, y + length * 1.15,
                        x - width * 0.25, y + length * 0.85,
                        hemX, hemY);
        ...
        // Remember the caller's outline settings so the crease lines don't clobber them
        javafx.scene.paint.Paint savedStroke = gc.getStroke();
        double savedWidth = gc.getLineWidth();

        // Inner shadow: a soft darker band along the lower half, where the fabric folds under
        gc.setFill(shadow);
        gc.beginPath();
        gc.moveTo(hemX, hemY);
        gc.bezierCurveTo(x - width * 0.30, y + length * 0.55,
                        x - width * 0.65, y + length * 0.45,
                        tailEndX + waveStrength, tailEndY - length * 0.10);
        gc.bezierCurveTo(x - width * 0.55, y + length * 1.05,
                        x - width * 0.25, y + length * 0.85,
                        hemX, hemY);
        gc.closePath();
        gc.fill();

        // Fold creases: lines fanning out from the shoulder toward the trailing tails
        gc.setStroke(crease);
        gc.setLineWidth(1.2);
        for (int i = 0; i < numWaves + 2; i++) {
            double t = (i + 0.5) / (numWaves + 2);
            double startX = neckX, startY = neckY + (hemY - neckY) * t;
            double endX = leadTipX + (tailEndX - leadTipX) * t + waveStrength * 0.5;
            double endY = leadTipY + (tailEndY - leadTipY) * t;
            double ctrlX = (startX + endX) / 2;
            double ctrlY = (startY + endY) / 2 - length * (0.18 - t * 0.22); // creases bow with the sweep
            gc.beginPath();
